...
export function HeroMainCard() {
  ...
  return (
    ...
    >
      <p className="font-display text-xs font-semibold uppercase tracking-[0.2em] text-ink-muted">{t.hero.badge}</p>
      ...
      <div className="mt-10 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
        <Button variant="primary" href={mailtoHref(locale, true)} className="min-h-12 px-8 py-3 text-sm">
          {t.hero.ctaPrimary}
        </Button>
        <Button variant="secondary" href={NAV_HREF.cases} className="min-h-12 px-8 py-3 text-sm">
          {t.hero.ctaDemo}
        </Button>
      </div>
    </BentoCard>
...
export function HeroSideCard() {
  ...
  return (
    <BentoCard className="flex flex-col justify-between md:col-span-4 md:col-start-9 md:row-start-1">
      ...
      <div
        id="demo"
        className="mt-8 scroll-mt-28 rounded-2xl border border-[color:var(--panel-border)] bg-[color:var(--panel-bg)] p-4 backdrop-blur-md"
      >
        ...
        <p className="font-display mt-2 text-xs text-ink-muted">{t.hero.demoHint}</p>
      </div>
    </BentoCard>
  );
}
